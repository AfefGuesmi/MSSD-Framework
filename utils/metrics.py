...
'''
from sklearn.metrics import f1_score, precision_score, recall_score, accuracy_score, jaccard_score, hamming_loss, \
...
def confusion_matrix(y_gt, y_pred, labels):
    # IMPORTANT: pass label_ids explicitly to every sklearn metric call
    # below. Without this, sklearn infers the label set from whichever
    # classes actually appear in y_gt/y_pred -- fine on a large test set
    # where every class is guaranteed to show up, but silently wrong on a
    # smaller/custom split where a rare class (e.g. Natural Organic
    # Material) may be entirely absent: the confusion matrix would then
    # come back smaller than len(labels), causing a shape mismatch with
    # the fixed-length row/column headers below, and macro-averaged
    # metrics would silently average only over classes present instead of
    # counting the missing class as 0 -- inflating the score without any
    # error or warning.
    label_ids = list(range(len(labels)))

    # compute metrics
    cm = metr.confusion_matrix(y_gt, y_pred, labels=label_ids)
    f1_macro = metr.f1_score(y_gt, y_pred, labels=label_ids, average='macro', zero_division=0)
    mPA = metr.recall_score(y_gt, y_pred, labels=label_ids, average='macro', zero_division=0)
    OA = metr.accuracy_score(y_gt, y_pred)
    UA = metr.precision_score(y_gt, y_pred, labels=label_ids, average=None, zero_division=0)
    PA = metr.recall_score(y_gt, y_pred, labels=label_ids, average=None, zero_division=0)
    f1 = metr.f1_score(y_gt, y_pred, labels=label_ids, average=None, zero_division=0)
    IoC = metr.jaccard_score(y_gt, y_pred, labels=label_ids, average=None, zero_division=0)
    mIoC = metr.jaccard_score(y_gt, y_pred, labels=label_ids, average='macro', zero_division=0)
